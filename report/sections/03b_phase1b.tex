...
\subsection{P1B result visualization}
Figure~\ref{fig:p1b_result} condenses the two most important robustness results: the boundary of the approximately known-pose assumption and the particle-count/runtime trade-off. The local PF remains effective for small-to-moderate initialization errors, while large prior errors can produce wrong-mode failures; increasing particle count beyond a few thousand gives diminishing RMSE improvement while runtime continues to increase.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/phase1/p1b_known_pose_robustness.pdf}
    \caption{P1B known-pose robustness. (a) Mean position RMSE versus increasing initial-state uncertainty for dead reckoning and PF+UWB. (b) PF position RMSE and runtime versus particle count.}
