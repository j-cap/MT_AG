...
\subsection{P1C result visualization}
Figure~\ref{fig:p1c_result} shows why P1C is interpreted as a global-mode-selection problem rather than a single poor hyperparameter choice. Neither increasing the particle population nor changing the conventional resampling threshold produces a monotonic transition to reliable pose recovery.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/phase1/p1c_global_search_sensitivity.pdf}
    \caption{P1C unknown-pose bootstrap-PF sensitivity. (a) Late position RMSE and terminal pose-success fraction versus particle count. (b) The same quantities versus resampling threshold $\gamma$.}
    \label{fig:p1c_result}
\end{figure}
