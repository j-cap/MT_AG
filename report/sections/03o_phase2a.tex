...
\section{Phase-2 experiment roadmap}\label{sec:phase2_plan}
The experiments build on one four-node, trajectory-first benchmark with known initial poses. P2A--P2F form the main infrastructure-free sequence: all mobile nodes remain uncertain, and UWB measures only inter-node distances. P2G is a separate extension with a continuing global position reference. Its letter follows the six main steps; it is not the next experiment to run.

\begin{itemize}
    \item \textbf{P2A -- benchmark (completed):} Establish the four-node simulation, IMU-only baseline, and all-six-link \SI{10}{Hz} UWB reference over 20 matched seeds. Decompose absolute position error into fleet-centroid drift and relative-shape error.
    \item \textbf{P2B -- uniform rate sweep (completed):} Reduce the update rate of all six links together from \SI{10}{Hz} through \(5,2,1,0.5\) Hz and the zero-UWB baseline to measure the first error-versus-communication curve.
    \item \textbf{P2C -- link selection (completed):} At equal numbers of UWB exchanges and identical per-tick opportunities, compare cyclic, random, geometry-aware, and information-aware choices of which pair to range.
    \item \textbf{P2D -- reliability (next):} Test packet loss, burst outages, range outliers, and NLOS-like positive bias. Evaluate measurement reliability separately from the initial scheduling comparison.
    \item \textbf{P2E -- IMU sensitivity:} Vary inertial noise and bias at representative communication budgets to see how sensor quality changes the required ranging rate.
    \item \textbf{P2F -- consolidated frontier:} Combine accuracy, worst-node error, and UWB exchange count across the infrastructure-free conditions; incorporate measured transaction energy when available.
    \item \textbf{P2G -- global-reference extension (optional, later):} Give one moving node a position known throughout the run, first exactly and then through noisy or intermittent reports, to assess how a global reference changes the frontier.
\end{itemize}

For the first P2G comparison, one of the existing four moving nodes becomes the known-position reference while trajectories, the other nodes' IMU realizations, seeds, six UWB links, and exchange schedule stay matched to P2A. Both conditions must be scored on the same three non-reference nodes so that a zero-error reference does not mechanically reduce fleet RMSE. The ideal-reference case bounds the value of global position information; later noisy-reference and outage cases must propagate its uncertainty and report reference-update costs separately from pairwise UWB exchanges. A position known only initially is already part of P2A and is not a new reference condition.
...
Figure~\ref{fig:p2a_node_spread} resolves this comparison by node. At each time instant, the solid or dashed curve is the mean position-error magnitude over the same 20 seeds, and the shaded area spans one sample standard deviation on either side, truncated at zero because error magnitude cannot be negative. The band describes variation between stochastic sensor and initial-velocity realizations; it is neither a confidence interval for the mean nor a bound on individual runs. In particular, the single realization in Fig.~\ref{fig:p2a_rep_errors} can locally favor IMU-only for Node~1 even when cooperation reduces its typical error across seeds. The paired fleet-level result remains the appropriate basis for the overall performance claim.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/phase2/p2a_node_error_spread.pdf}
    \caption{P2A per-node position-error histories across 20 matched stochastic seeds. Curves show pointwise mean error magnitude; shading shows mean \(\pm\) one sample standard deviation (lower edge clipped at zero). The true trajectories and UWB schedule are fixed across seeds.}
    \label{fig:p2a_node_spread}
\end{figure}
