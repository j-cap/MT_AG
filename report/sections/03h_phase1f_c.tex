...
\subsection{P1F-C result visualization}
Figure~\ref{fig:p1fc_result} visualizes why the parameter freeze is safety-aware rather than based only on correct-mode concentration. The selected tuple $(\alpha,\beta,c_\lambda)=(0.5,0,2)$ lies in the high-correct-lock, low-wrong-lock region while limiting catastrophic ancestry collapse sufficiently to pass the predefined freeze rule.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/phase1/p1fc_tuning_tradeoff.pdf}
    \caption{P1F-C development sweep. Correct-mode lock is shown against (a) catastrophic ancestry-collapse fraction and (b) wrong-mode-lock fraction. The star denotes the frozen tuple $(0.5,0,2)$.}
    \label{fig:p1fc_result}
\end{figure}
