...
\subsection{P1F-D result visualization}
Figure~\ref{fig:p1fd_result} shows the held-out matched error comparison at the primary $N_p=1200$ budget. AACOPF often yields lower late position/yaw error, but strict pose success remains worse than conventional PF. The figure therefore supports the limited error-shaping claim rather than a convergence claim.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/phase1/p1fd_matched_pf_aacopf.pdf}
    \caption{P1F-D matched PF--AACOPF comparison at $N_p=1200$. Points below the dashed equality line correspond to seeds for which AACOPF has lower late error.}
    \label{fig:p1fd_result}
\end{figure}
