...
\subsection{P1F-F result visualization}
Figure~\ref{fig:p1ff_result} condenses the ranging-stress campaign across the balanced and minority-correct clouds. Conventional PF retains a higher correct-lock fraction and a lower wrong-lock fraction for every corruption family, so literal AACOPF cannot be credited with inherent outlier/NLOS robustness under the unchanged Gaussian likelihood.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/phase1/p1ff_ranging_stress.pdf}
    \caption{P1F-F UWB ranging stress. Fractions are averaged over the balanced and minority-correct controlled clouds. (a) Correct-mode lock fraction and (b) wrong-mode lock fraction for clean, impulse, mixture, and NLOS-burst conditions.}
    \label{fig:p1ff_result}
\end{figure}
