...
This chapter defines the abbreviations and mathematical notation used throughout the report. The notation tables are intended to be exhaustive for the current version. When a later experiment introduces a new abbreviation or symbol, it should be added here and also explained at its first use in the corresponding chapter.
...
$j$ & Generic secondary index, e.g. a summation index or an index used in the observability horizon.\\
$K$ & Final discrete-time index of a finite analysis/observability horizon.\\
$t$ & Continuous time in seconds.\\
$t_k=k\Delta t$ & Continuous time associated with discrete sample $k$ for the fixed-rate simulations.\\
$T$ & Total duration of a simulation experiment.\\
$\Delta t$ & Sampling interval between discrete states.\\
$A$ & Label used for the auxiliary UWB node in the current single-auxiliary-node formulation.\\
...
$p(\cdot)$ & Probability density or conditional probability density; context distinguishes it from the position vector $\bm p$.\\
$\delta(\cdot)$ & Dirac delta distribution used in the particle representation of a probability density.\\
$\operatorname{wrap}(\theta)$ & Mapping of a generic angle $\theta$ to the interval $[-\pi,\pi)$.\\
$\operatorname{atan2}(y,x)$ & Four-quadrant inverse tangent used for the circular mean of yaw.\\
$I_q$ & $q\times q$ identity matrix; $q$ denotes the matrix dimension.\\
$0_{r\times c}$ & $r\times c$ zero matrix; $r$ and $c$ denote row and column dimensions.\\
\bottomrule
\end{longtable}
...
$p_{A,x,k}$ & Navigation-frame $x$-coordinate of auxiliary node $A$.\\
$p_{A,y,k}$ & Navigation-frame $y$-coordinate of auxiliary node $A$.\\
...
$\bm{v}_k^{(i)}$ & Velocity component of particle $i$.\\
$\psi_k^{(i)}$ & Yaw component of particle $i$.\\
$w_k^{(i)}$ & Normalized importance weight of particle $i$.\\
$\tilde w_k^{(i)}$ & Unnormalized importance weight of particle $i$ before normalization.\\
$\bm{a}_{k}^{b,(i)}$ & Body-frame acceleration used to propagate particle $i$.\\
$\omega_k^{(i)}$ & Yaw rate used to propagate particle $i$.\\
...
$\hat{\bm{v}}_k$ & Estimated velocity component of $\hat{\bm{x}}_k$.\\
...
$e_{p,95}$ & Empirical 95th-percentile position error.\\
