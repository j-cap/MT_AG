...
from mt_ag.simulation import generate_curved_trajectory, auxiliary_trajectory, integrate_dr, noisy_dr_increments
from mt_ag.sensors import generate_uwb_ranges
from mt_ag.particle_filter import run_bootstrap_pf
from mt_ag.metrics import summary_metrics, position_error
from mt_ag.observability import singular_values
...
def main():
    ...
    noisy_inc = noisy_dr_increments(tr.increments, rng, dr_cfg["sigma_distance_m"],
                                    dr_cfg["sigma_heading_rad"], dr_cfg["heading_bias_rw_rad"])
    ...
    pf = run_bootstrap_pf(noisy_inc, z, aux_moving, tr.state[0], pf_cfg["initial_std"], rng,
                          pf_cfg["n_particles"], pf_cfg["sigma_process_distance_m"],
                          pf_cfg["sigma_process_heading_rad"], uwb_sigma,
                          pf_cfg["resample_fraction"])

    metrics = {"ideal_dr": summary_metrics(ideal_dr, tr.state),
    ...
    np.savetxt(out / "trajectory_report.csv", data[::10], delimiter=",", header=header, comments="")
    ...
    ax.set_xlabel("x [m]"); ax.set_ylabel("y [m]"); ax.axis("equal"); ax.grid(True, alpha=.3); ax.legend()
    fig.tight_layout(); fig.savefig(figdir / "trajectory.pdf"); fig.savefig(figdir / "trajectory.png", dpi=180); plt.close(fig)
    ...
    ax.set_xlabel("time [s]"); ax.set_ylabel("position error [m]"); ax.grid(True, alpha=.3); ax.legend()
    fig.tight_layout(); fig.savefig(figdir / "position_error.pdf"); fig.savefig(figdir / "position_error.png", dpi=180); plt.close(fig)
